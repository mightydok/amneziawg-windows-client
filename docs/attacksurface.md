...
### Geo-split routing lists

The manager service downloads country prefix lists from the configured sources (by default `raw.githubusercontent.com`, project `ipverse/country-ip-blocks`) over TLS through WinHTTP and stores them in `C:\Program Files\AmneziaWG\Data\geo`, which inherits the `Data` DACL, so only Local System and Administrators can read or modify them; `settings.json` in the same directory is written only through the manager IPC, which requires an elevated UI session. The sources publish no signatures, so a list is accepted only if every line parses as a CIDR, it has at least 2000 IPv4 (200 IPv6) prefixes, it is smaller than 4 MiB, and its prefix count did not change by more than 50 % compared to the previous download; otherwise the previous list stays in use. A malicious list can therefore at most steer traffic to the listed prefixes outside the tunnel (the same effect as adding them to "always directly"), it cannot open inbound ports or execute anything. The tunnel service parses the list before dropping privileges; parsing is plain Go with bounded input. The UI can trigger a download at any time; a non-administrator session with `LimitedOperatorUI` cannot change settings or trigger downloads.
